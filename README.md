...
`ROBOT_PROFILE` 只能是 `492X` 或 `492Z`。切换机器人时必须先执行
`make clean`，避免复用上一台机器的目标文件。PROS 默认值位于根目录
[`Makefile`](Makefile)，当前为 `492Z`；PC 默认值位于
[`CMakeLists.txt`](CMakeLists.txt)。身份、端口和飞行记录元数据由同一个
选择同时切换，端口表见 [`robot_profiles.hpp`](include/robot/config/robot_profiles.hpp)
和 [`config/robots/`](config/robots)。
...
4. 左摇杆 `Y` 控制前后，`X` 控制转向；油门采用线性曲线，满杆会在一个名义 10 ms 控制帧内达到 12 V 命令上限。V5 电池满电端电压可高于 12 V，但 PROS `move_voltage` 的合法命令上限仍是 12000 mV，程序不会写 12.8 V 或做 `12 / battery_V` 放大。
5. 每次开机前必须把 Lift 放在物理最低点。初始化会把所选配置的 Lift 内置编码器清零（492X 为 Port 19/18，492Z 为 Port 14/5）；右摇杆 `Y`（Axis2）向上/向下近似线性控制 Lift。松杆、断控、数据异常和限位保护均为 `Hold`。
6. 任意异常立即按住 `B` 或直接禁用。按住 `B` 时六个底盘马达立即 `Coast`；它不锁存，松开后会按当前摇杆位置恢复输出。
7. 先用约 10% 杆量确认六个轮端方向、无机械对抗和 `Coast` 行为，再落地；Lift 也必须先架空低杆量确认正方向和两侧同步，再逐段测试限位。

底盘在中立、断控、场控连接、禁用或看门狗触发时走 `stop(Coast)`；Lift 独立走 `stop(Hold)`，并由双内置编码器执行行程保护。Lift 的物理最大行程为 830 motor deg，当前软件在 820 deg 停止继续上升，并在接近上下端时连续降低输出。没有 IMU，所以航向辅助、位姿和自动控制全部关闭。
...
| [`include/robot/lift/`](include/robot/lift)          | Lift Axis2 映射、双编码器一致性和行程安全门          |
...
| API                                                                          | 用途                                 | 定义位置                                                        |
| ---------------------------------------------------------------------------- | ---------------------------------- | ----------------------------------------------------------- |
| `robot::make492XRobotConfig()` / `robot::make492ZRobotConfig()`               | 生成指定机器人的身份与端口配置                    | [`robot_profiles.hpp`](include/robot/config/robot_profiles.hpp) |
| `robot::makeSelectedRobotConfig()` / `robot::selectedRobotId()`               | 读取编译期选中的完整配置和身份                    | 同上                                                          |
| `robot::makeOfflineRobotConfig()`                                             | PC 夹具使用的所选配置兼容入口                    | 同上                                                          |
...
| `robot::CommissioningControlCycle` | 12 V 单杆 Curvature / Quick Turn 真机调试链 | [`commissioning_curvature.hpp`](include/robot/manual/commissioning_curvature.hpp)          |
...
| 参数组  | 字段                                                                                   | 修改位置                                                                              | 当前状态/来源                                                                |
| ---- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 身份   | `robot_id`、schema、revision                                                           | [`robot_profiles.hpp`](include/robot/config/robot_profiles.hpp)                   | 492X/492Z 身份随编译配置选择                                                  |
| 马达   | port、`reversed`、`cartridge_rpm`、逐马达 `motor_rev_per_wheel_rev`                        | 同上                                                                                | 只有 Smart Port 因机器人而异；方向、齿比和其余参数共用，仍需分别架空复测确认                         |
...
| 试车驾驶 | `throttle_shape`、`curvature_gain`、`quick_turn_gain`、`quick_turn_enter/exit_throttle` | [`commissioning_curvature.hpp`](include/robot/manual/commissioning_curvature.hpp) | 左 Y 线性油门、左 X Curvature；低油门自动 Quick Turn；全增益 1.0，0.15/0.25 滞回门限待 HIL 调整 |
| 试车加速 | 输入 rise/fall、`output_slew` rise/fall                                                 | 同上                                                                                | 激进候选 100/s、1200 V/s；名义一个 10 ms 控制帧内达到 12 V，待新一轮 HIL 复测                 |
| Lift 试车 | Axis2 死区/线性权重、最大电压、端点余量、减速区、双编码器差值阈值                                      | [`lift_control.hpp`](include/robot/lift/lift_control.hpp)                           | 当前 0.06/线性、12 V、10/90/25 deg，全部待 HIL 复核                               |
| 能力开关 | `RobotCapabilities`                                                                  | `RobotConfig` / 同上                                                                | 当前全部锁定                                                                 |

[`config/hardware_profile.yaml`](config/hardware_profile.yaml) 是双配置索引；
[`config/robots/492X.yaml`](config/robots/492X.yaml) 与
[`config/robots/492Z.yaml`](config/robots/492Z.yaml) 是两台机器的硬件事实清单。
其中的 `null` 表示仍需实机或 CAD 数据，不应随意填入“看起来合理”的值。
...
| 样机试车            | 左 Y/X Curvature、低油门自动 Quick Turn、B 按住 Coast、12 V、Slew、TTL | [`CommissioningCurvatureConfig`](include/robot/manual/commissioning_curvature.hpp) |
