...
| 试车驾驶 | `curvature_gain`、`quick_turn_gain`、`quick_turn_enter/exit_throttle` | [`commissioning_curvature.hpp`](include/robot/manual/commissioning_curvature.hpp) | 左 Y/X 单杆 Curvature；低油门自动 Quick Turn；全增益 1.0，0.15/0.25 滞回门限待 HIL 调整 |
| 试车加速 | 输入 rise/fall、`output_slew` rise/fall | 同上 | 激进候选 20/s、240 V/s；名义 0→12 V 约 0.05 s，待新一轮 HIL 复测 |
