...
> 当前状态：已生成可下载的 **12 V 全电压 commissioning 版本**，进入非场控 Driver Control 后可直接用左摇杆单杆 Arcade 测试底盘最大命令功率，无组合键解锁。六个底盘马达的零输出策略统一为 `Coast`，按住 `B` 会立即松弛，松开后恢复摇杆控制。此前“前推后退”的真机反馈已通过翻转六个静态 `reversed` 配置修正，但必须先架空低杆量复测方向，再进行落地全幅测试。官方 `hardware_output`、`driver_control` 与全部自动能力仍保持 `false`；`autonomous()` 始终执行 `DoNothing`。
