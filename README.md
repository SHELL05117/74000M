...
> 当前状态：已生成可下载的 **12 V 全电压 commissioning 版本**，进入非场控 Driver Control 后可直接用左摇杆单杆 Arcade 测试底盘最大命令功率，无组合键解锁。根据首轮实测“加速度偏慢”的反馈，直线油门 0→12 V 的名义爬升时间已由约 0.33 s 缩短为约 **0.05 s**；这是待复测的快速 HIL 候选。六个底盘马达的零输出策略统一为 `Coast`，按住 `B` 会立即松弛，松开后恢复摇杆控制。此前“前推后退”的真机反馈已通过翻转六个静态 `reversed` 配置修正，但必须先架空低杆量复测方向，再进行落地全幅测试。官方 `hardware_output`、`driver_control` 与全部自动能力仍保持 `false`；`autonomous()` 始终执行 `DoNothing`。
...
| 受控试车输出 | 非场控下可用，最大 `12 V` | [`CommissioningControlCycle`](include/robot/manual/commissioning_arcade.hpp) |
...
|---|---:|---:|---|---:|---:|---:|
| 左前 LA | 11 | 600 RPM | 36T → 48T | `4/3` | 450 RPM | `false` |
| 左中 LM | 12 | 200 RPM | 36T → 12T；同轴 36T → 48T | `4/9` | 450 RPM | `true` |
| 左后 LB | 13 | 600 RPM | 36T → 48T | `4/3` | 450 RPM | `true` |
| 右前 RA | 1 | 600 RPM | 36T → 48T | `4/3` | 450 RPM | `true` |
| 右中 RM | 2 | 200 RPM | 36T → 12T；同轴 36T → 48T | `4/9` | 450 RPM | `false` |
| 右后 RB | 3 | 600 RPM | 36T → 48T | `4/3` | 450 RPM | `false` |
...
4. 左摇杆 `Y` 控制前后，`X` 控制转向；满杆会在名义约 0.05 s 后达到 12 V 命令上限。V5 电池满电端电压可高于 12 V，但 PROS `move_voltage` 的合法命令上限仍是 12000 mV，程序不会写 12.8 V 或做 `12 / battery_V` 放大。
