...
### 5.2 修改 492X / 492Z 电机端口

修改电机端口时，需要同步维护以下文件：

1. **实际参与固件编译的配置**：[`include/robot/config/robot_profiles.hpp`](include/robot/config/robot_profiles.hpp)。
   492X 修改 `make492XHardwareConfig()`，492Z 修改 `make492ZHardwareConfig()`；
   `motorFromReversedFlag()` 的第一个参数是 Smart Port。不要在这里只交换端口后遗漏其他记录。
2. **对应机器的硬件清单**：[`config/robots/492X.yaml`](config/robots/492X.yaml)
   或 [`config/robots/492Z.yaml`](config/robots/492Z.yaml)。同步修改对应马达的 `smart_port`。
3. **端口回归测试**：[`tests/test_config.cpp`](tests/test_config.cpp)。同步更新
   `x_ports` 或 `z_ports` 数组，否则 PC 测试会报告源码端口表与预期不一致。
4. **人工查看用端口表**：[`docs/ROBOT_PROFILES.md`](docs/ROBOT_PROFILES.md)。同步更新表格，避免调试人员按旧接口接线。

只修改端口时，不需要修改 `.robot-profile.local.mk`、`Makefile`、`src/main.cpp`
或驾驶/Lift 控制核心代码；`.robot-profile.local.mk` 只负责记住当前选择的机器人。

修改完成后，使用快捷命令自动清理旧目标并重新构建：
...
第一次上机必须架空逐端口确认方向。配置文件和 PC 测试通过不能代替 492X/492Z
各自的真机方向与机构一致性验证。
