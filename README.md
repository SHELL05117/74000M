...
> 当前状态：已生成可下载的 **12 V 全电压 commissioning 版本**，可用左摇杆单杆 Arcade 测试底盘最大命令功率。此前“前推后退”的真机反馈已通过翻转六个静态 `reversed` 配置修正，但必须先架空低杆量复测方向，再进行落地全幅测试。官方 `hardware_output`、`driver_control` 与全部自动能力仍保持 `false`；`autonomous()` 始终执行 `DoNothing`。
...
4. 左摇杆 `Y` 控制前后，`X` 控制转向；满杆可在 Slew 后达到 12 V 命令上限。
5. 任意异常立即按 `B`。`B` 是锁存停车，必须禁用再重新启用并重新执行解锁组合键。
6. 先用约 10% 杆量确认六个轮端方向、无机械对抗和 Brake 行为，再落地；全幅测试必须留足加速与停车距离。
...
| 样机试车 | 解锁组合键、B 停车、左 Y/X、12 V、Slew、TTL | [`CommissioningArcadeConfig`](include/robot/manual/commissioning_arcade.hpp) |
